...
# 此脚本确保Skills目录中的分解规格文件与主DEV_SPEC.md文件保持同步。

# 功能：
# - **自动检测**：自动检测所有一级章节（## N. Title）
# - **智能命名**：根据中文标题生成基于slug的文件名
# - **哈希基于同步**：仅当DEV_SPEC.md实际更改时才重新生成
# - **动态索引**：根据检测到的章节生成SPEC_INDEX.md

# 用法：
#     python sync_spec.py [--force]

# 选项：
#     --force    强制重新生成，即使哈希匹配也如此
import hashlib
import re
import sys
...
class SpecSynchronizer:
    ...
    def needs_sync(self, force: bool = False) -> bool:
        ...
        if stored_hash is None:
            print("[WARN] No stored hash found. First-time sync required.")
            return True
        
        if current_hash != stored_hash:
            print("[INFO] DEV_SPEC.md has changed. Re-sync required.")
            return True
        
        print("[OK] Spec files are up to date.")
        return False
    ...
    def _generate_slug(self, cn_title: str) -> str:
        ...
        print(f"[WARN] No slug mapping for '{cn_title}'. Using sanitized title.")
        print("       Tip: Add this to TITLE_SLUG_MAP for better filenames.")
    ...
    def split_spec(self):
        ...
        if orphaned_files:
            print(f"\nRemoving {len(orphaned_files)} orphaned file(s)...")
